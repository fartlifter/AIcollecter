...
# === 사용자 원본 GEM 지침 기반 수정본 (날짜 필수 & 검찰청 구체 명시 반영) ===
...
① 첫 문장 구조 (날짜 필수 & 주격 조사 생략 및 쉼표 대체):
   ...
   - 발생 시점(예: 2일, 지난달 31일 등)을 기사 본문에서 찾아 반드시 정확히 명시한다.
...
③ 기관별 주어 표기 세부 규칙:
   ...
   - 검찰이 주어인 경우:
     * 막연히 '검찰'이라 쓰지 않고, 반드시 기사에 명시된 구체적 검찰청(예: 서울중앙지검, 서울북부지검, 수원지검 등) 또는 수사단/특검팀(예: 내란 특검팀)을 명시한다. (예: 서울북부지검, 2일 / 수원지검 형사6부, 지난달 31일)
