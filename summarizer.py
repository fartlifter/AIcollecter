...
[4. 문장별 상세 구성 및 사실 오해 방지 날짜 표기법 (핵심 준수)]
① 첫 문장 구조 (행위 발생 시점 엄격 일치 & 주격조사 생략):
   - 반드시 `[주체], [실제 행위 발생 시점] [내용 요약(어근 마무리)].` 순으로 작성한다.
   - 첫 번째 문장의 주어 뒤에만 주격 조사(이/가/은/는)를 생략하고 쉼표(,)를 사용한다.
   - [날짜 표기 사실 오해 방지 절대 규칙]:
     * 기사 서두의 전언 시점(예: "N일 법조계에 따르면", "N일 취재에 따르면", "N일 언론에 따르면")에 등장하는 날짜를 주어의 행위 날짜로 오인해 사용하지 않는다.
     * 반드시 '주어가 해당 판결·결정·조치·수사 등의 행위를 실제로 행한 날짜'를 본문에서 찾아 기재한다.
     * 본문에 주어의 실제 행위 시점이 '지난달 20일', '지난해 12월', '최근' 등으로 명시되어 있다면 해당 표현을 그대로 반영한다. (기사 작성일인 'N일'로 둔갑 금지)
   - [세계일보 날짜 표기]: 월과 일은 반드시 붙여 쓴다. (예: 3일, 9월2일, 지난달 31일 등 / 9월 2일 금지)
   - 주어는 기사 1문단의 당사자가 아니라 판결·결정·수사를 내린 사법 주체(재판부, 검찰청, 특검팀, 변호사 등)로 설정한다.
